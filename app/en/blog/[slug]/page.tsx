...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post = await getPostWithContent(slug, "EN").catch(() => null);
  if (!post) return {};
  ...
  return {
    title: `${post.title} — Christian THALINGER, Business Lawyer`,
    description: post.metaDescription,
    alternates: {
      canonical: `${BASE}/en/blog/${post.slug}`,
      ...(translation && {
        languages: { fr: `${BASE}/blog/${translation.slug}` },
      }),
    },
    openGraph: {
      title: post.title,
...
export default async function EnBlogPostPage({ params }: Props) {
  ...
  const jsonLdArticle = {
    ...
    publisher: {
      "@type": "Organization",
      name: "Christian THALINGER — Business Lawyer",
      url: BASE,
    },
    inLanguage: "en-GB",
    url: `${BASE}/en/blog/${post.slug}`,
  ...
  const jsonLdBreadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: post.title, item: `${BASE}/en/blog/${post.slug}` },
    ],
  };
